...
- The `:csp_nonce_assign_key` nonce is now also applied to server-rendered responses. The page data `<script>` tag in SSR bodies is generated by the client-side `buildSSRBody` helper (which doesn't currently support nonces), so the nonce attribute is injected server-side. The configured nonce is authoritative, since it is the same value apps use to build their `Content-Security-Policy` header: if the tag already carries it (e.g. the client-side libraries gain native nonce support), the body passes through untouched, and any other nonce on the tag is replaced. Previously the nonce was only applied to client-side rendered responses.
