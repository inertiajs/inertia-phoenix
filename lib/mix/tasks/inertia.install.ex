...
if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.Inertia.Install do
    ...
    defp inertia_root_html(framework) do
      # svelte generates a css file from the css in the components, so we need to include a second css
      # file besides the one generated by tailwindcss.
      svelte_css =
        if framework == "svelte",
    ...
    def update_esbuild_config(igniter) do
      case igniter.args.options[:client_framework] do
        ...
        "svelte" ->
          ...
          igniter
          # Remove existing esbuild config since the entire dependency will be deleted
          |> Igniter.Project.Config.remove_application_configuration("config.exs", :esbuild)
          # Add a watcher for esbuild using node instead of the esbuild cli
          |> Igniter.Project.Config.configure(
            "dev.exs",
            Igniter.Project.Application.app_name(igniter),
            [endpoint, :watchers],
          ...
          # Remove esbuild dep as we use our own config
          |> Igniter.Project.Deps.remove_dep(:esbuild)
          # The aliases in the mix.exs file are updated to use esbuild.config.js instead of the cli
